...
def ingresar_matriz():
    ...
    matriz_window.configure(bg="#F2F2F2")  # Fondo pastel suave
    ...
    frame_matriz_a = tk.Frame(matriz_window, bg="#FFDDC1", padx=10, pady=10)  # Fondo pastel
    frame_matriz_b = tk.Frame(matriz_window, bg="#FFDDC1", padx=10, pady=10)  # Fondo pastel
    frame_resultados = tk.Frame(matriz_window, bg="#FFDDC1", padx=10, pady=10)  # Fondo pastel
    frame_botones = tk.Frame(matriz_window, bg="#FFDDC1", padx=10, pady=10)  # Fondo pastel
    ...
    tk.Label(frame_matriz_a, text="Matriz A", bg="#FFDDC1", font=("Arial", 10, "bold")).grid(row=0, column=0, columnspan=n)
    for i in range(n):
        fila_entries = []
        for j in range(n):
    ...
    tk.Label(frame_matriz_b, text="Matriz B (para multiplicación)", bg="#FFDDC1", font=("Arial", 10, "bold")).grid(row=0, column=0, columnspan=n)
    for i in range(n):
        fila_entries = []
        for j in range(n):
    ...
    tk.Label(frame_resultados, text="Resultados B (para Gauss/Cramer)", bg="#FFDDC1", font=("Arial", 10, "bold")).grid(row=0, column=0, columnspan=n)
    for i in range(n):
    ...
    boton_color_morado = "#9C27B0"
    boton_color_naranja = "#FF9800"
    boton_color_rosado = "#E91E63"
    boton_color_amarillo = "#FFEB3B"
    texto_boton = "#FFFFFF"

    tk.Button(frame_botones, text="Inversa de la matriz A", bg=boton_color_morado, fg=texto_boton, command=lambda: calcular_inversa(n)).grid(row=0, column=0, padx=5, pady=5)
    tk.Button(frame_botones, text="Multiplicar matrices", bg=boton_color_naranja, fg=texto_boton, command=lambda: calcular_multiplicacion(n)).grid(row=0, column=1, padx=5, pady=5)
    tk.Button(frame_botones, text="Resolver ecuaciones (Gauss)", bg=boton_color_rosado, fg=texto_boton, command=lambda: calcular_ecuaciones(n, metodo='gauss')).grid(row=0, column=2, padx=5, pady=5)
    tk.Button(frame_botones, text="Resolver ecuaciones (Cramer)", bg=boton_color_amarillo, fg=texto_boton, command=lambda: calcular_ecuaciones(n, metodo='cramer')).grid(row=1, column=0, columnspan=3, pady=5)

    # Botón para regresar a la ventana principal
    tk.Button(frame_botones, text="Regresar", bg="#f44336", fg="#FFFFFF", command=matriz_window.destroy).grid(row=2, column=0, columnspan=3, pady=10)
...
root.configure(bg="#F2F2F2")  # Fondo pastel suave

tk.Label(root, text="Tamaño de la matriz (n):", bg="#F2F2F2", fg="#333333", font=("Arial", 12, "bold")).pack(pady=10)
entry_n = tk.Entry(root)
entry_n.pack(pady=5)
tk.Button(root, text="Ingresar Matrices", bg="#FF5722", fg="#FFFFFF", command=ingresar_matriz).pack(pady=20)  # Botón vibrante
